    ...
    reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class)
  	...
  	public void pay(ReadableMap options, Promise promise) {
        ...
        for (int i = 0; i < split.length; ++i) {
            String resultInfo = split[i];
            if (resultInfo.contains("resultStatus")) {
                String[] results = resultInfo.replace("{", "").replace("}", "").split("=");
                if (results.length > 1) {
                    resultCode = results[1];
                }
            }
        }
        sendEvent(mReactContext, "rnAlipayCallback", resultCode.equals("9000") ? true : false);
    }
}